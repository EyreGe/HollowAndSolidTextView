...
/**
 * Created by yuanyuan on 2017/10/16.
 */

public class HollowText extends TextView {
    private static final int STROKE_WIDTH = 2;
    private int borderCol;

    private Paint borderPaint;

    public HollowText(Context context, AttributeSet attrs) {
        super(context, attrs);

        TypedArray a = context.getTheme().obtainStyledAttributes(attrs,
                R.styleable.TextBorder, 0, 0);
        try {
            borderCol = a.getInteger(R.styleable.TextBorder_borderColor, 0);//0 is default
        } finally {
            a.recycle();
        }
